-- ============================================
-- KOHLS ADMIN HOUSE X – FINAL (FIXED, FULL SCRIPT)
-- Custom Tool Kick with CreatePart + SyncResize (7x7x7) + SyncAnchor
-- NO SWORD commands. All monitors, clear, antibh, pad claimer intact.
-- ============================================
...
if not Rayfield then
    error("Rayfield failed to load – check internet connection.")
end
...
local function KickPlayer(target)
    ...
    local success, err = pcall(function()
        ...
        if not createOk then error("CreatePart failed: " .. tostring(createErr)) end
        ...
        if not tool then error("Created Tool not found.") end

        local handle = tool:FindFirstChild("Handle")
        if not handle or not handle:IsA("BasePart") then error("Tool has no Handle.") end

        -- Resize
        local resizeOk, resizeErr = pcall(function()
...
local function clearAll()
    ...
    local success, err = pcall(function()
        ...
        print("[.clr] Found " .. #instances .. " instances. Deleting in batches...")
        local total = 0
...
MiscTab:CreateButton({
    ...
    Callback = function()
        ...
        task.wait(0.1)
        notify(".kick", "speed 0 → freeze → size nan → freeze me → create Tool at victim HRP → resize Handle to 7x7x7 → anchor → wait 0.5s → thaw me (NO SWORD)")
...
MiscTab:CreateButton({
    ...
    Callback = function()
        ...
        print(".kick <partial> – speed 0 → freeze → size nan → freeze me → create Tool at victim HRP → resize Handle to 7x7x7 → anchor → wait 0.5s → thaw me (NO SWORD)")
...
task.spawn(function()
    ...
    local notifications = {
        ...
        {".kick", "speed 0 → freeze → size nan → freeze me → create Tool at victim HRP → resize Handle to 7x7x7 → anchor → wait 0.5s → thaw me (NO SWORD)"},
...
print("Kick: speed 0 → freeze → size nan → freeze me → create Tool at victim HRP → resize Handle to 7x7x7 → anchor → wait 0.5s → thaw me (NO SWORD).")
