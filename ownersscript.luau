-- ============================================
-- KOHLS ADMIN HOUSE X – FIXED (CUSTOM TOOL KICK, NO SWORD)
-- ============================================
-- Based on the original working script. Only the KickPlayer function is replaced
-- with custom Tool creation (CreatePart, SyncResize 7x7x7, SyncAnchor).
-- All other features (monitors, clear, killbrick, pad claimer, etc.) are unchanged.
-- ============================================
...
-- ===== KICK SEQUENCE (CUSTOM TOOL, NO SWORD) =====
...
MiscTab:CreateButton({
   ...
   Callback = function()
      ...
      task.wait(0.1)
      notify(".kick", "speed 0 → freeze → size nan → freeze me → create Tool at victim HRP → resize Handle to 7x7x7 → anchor → wait 0.5s → thaw me")
...
MiscTab:CreateButton({
   ...
   Callback = function()
      ...
      print(".kick <partial> – speed 0 → freeze → size nan → freeze me → create Tool at victim HRP → resize Handle to 7x7x7 → anchor → wait 0.5s → thaw me")
...
task.spawn(function()
   ...
   local notifications = {
      ...
      {".kick", "speed 0 → freeze → size nan → freeze me → create Tool at victim HRP → resize Handle to 7x7x7 → anchor → wait 0.5s → thaw me"},
